...
private ["_mrkOrigen","_pos","_lado","_cuenta","_mrkDestino","_veh","_posOrigen","_ladosMalos","_posDestino","_tipoVeh",
"_tipoVehs","_tipoMuni","_size","_vehicle","_grupoVeh","_rondas","_objetivo","_objetivos","_tiempo"];
...
_tipoVehs = if (_lado == malos) then {vehNATOMRLS} else {vehCSATMRLS};
_vehChoices = [];
{
    if ([_x] call A3A_fnc_vehAvailable) then {_vehChoices pushBack _x };
} forEach _tipoVehs;
if (count _vehChoices == 0) exitWith {};
_tipoVeh = selectRandom _vehChoices;
...
_tipoMunis = getArtilleryAmmo [_veh];
_tipoMuni = _tipoMunis select 0;
_grupoVeh = _vehicle select 2;
_size = [_mrkDestino] call A3A_fnc_sizeMarker;

if (_posDestino inRangeOfArtillery [[_veh], _tipoMuni]) then {
	while {(alive _veh) and ({_x select 0 == _tipoMuni} count magazinesAmmo _veh > 0) and (_mrkDestino in forcedSpawn)} do {
		...
		if (!isNull _objetivo) then {
			...
			_tiempo = _veh getArtilleryETA [position _objetivo, _tipoMuni];
			sleep 9 + ((_rondas - 1) * 3);
		} else {
			sleep 29;
		};
...
//LET ARTY STICK AROUND FOR A WHILE BEFORE DESPAWN - CHANCE TO KILL AND CAPTURE
sleep 1800;
waitUntil {
    sleep 360;
    !([distanciaSPWN,1,_x,buenos] call A3A_fnc_distanceUnits) and (({_x distance _x <= distanciaSPWN} count (allPlayers - (entities "HeadlessClient_F"))) == 0)
};
{
    sleep 1;
