...
debug = true;//debug variable, not useful for everything..
...
minasAAF = if ((!hayRHS) and !hayIFA and !myCustomMod) then
	...
	if (hayRHS) then
		...
		{
		if (hayIFA and !myCustomMod) then { [
		    "LIB_PMD6_MINE_mag",
		    "LIB_TM44_MINE_mag",
...
arrayCivVeh = if !(hayIFA) then
	{
	["C_Hatchback_01_F","C_Hatchback_01_sport_F","C_Hatchback_01_F","C_Hatchback_01_sport_F","C_Offroad_01_F","C_Offroad_01_F","C_Offroad_01_repair_F","C_SUV_01_F","C_SUV_01_F","C_Van_01_box_F","C_Van_01_fuel_F","C_Van_01_transport_F","C_Truck_02_transport_F","C_Truck_02_covered_F","C_Offroad_02_unarmed_F","C_Truck_02_fuel_F","C_Truck_02_box_F","C_Van_02_transport_F","C_Van_02_vehicle_F","C_Van_02_medevac_F","C_Van_02_service_F"];
	}
else
	{
...
guidedLaunchersNATO = [];
guidedLaunchersCSAT = [];
guidedLauncherMunitionsNATO = [];
guidedLauncherMunitionsCSAT = [];
...
if (!hayIFA) then
	...
	if (!activeUSAF) then
		...
		{
		if (buenos == independent) then {call compile preProcessFileLineNumbers "Templates\malosRHSUSAF.sqf"}
		else {call compile preProcessFileLineNumbers "Templates\buenosRHSUSAF.sqf"};
		};
...
medics = SDKMedic + [
    (FIAsquad select ((count FIAsquad)-1)),
    (NATOSquad select ((count NATOSquad)-1)),
    (NATOSpecOp select ((count NATOSpecOp)-1)),
    (CSATSquad select ((count CSATSquad)-1)),
    (CSATSpecOp select ((count CSATSpecOp)-1))
];
...
vehFixedWing = vehNATOPlane + vehCSATPlane + vehNATOPlaneAA + vehCSATPlaneAA + [vehSDKPlane];
...
vehUnlimited = vehNATONormal + vehCSATNormal + [vehNATORBoat,vehNATOPatrolHeli,vehCSATRBoat,vehCSATPatrolHeli,vehNATOUAV,vehNATOUAVSmall,NATOMortar,vehCSATUAV,vehCSATUAVSmall,CSATMortar] + NATORaisedStatics + CSATRaisedStatics;
...
civBoats = if !(hayIFA) then {["C_Boat_Civil_01_F","C_Boat_Civil_01_rescue_F","C_Scooter_Transport_01_F","C_Boat_Transport_02_F","C_Rubberboat"]} else {[]};
...
fn_InitNumberAssets ={
	...
	_assetTotalNumber = _this select 1;
    _assetEachNumber = ceil(_assetTotalNumber/(count _assetTypes));
    {timer setVariable [_x,_assetEachNumber,true]} forEach _assetTypes;
};
...
//{timer setVariable [_x,0,true]} forEach (vehAttack + vehNATOAttackHelis + vehNATOPlane + vehCSATPlane + vehNATOPlaneAA + vehCSATPlaneAA + vehCSATAttackHelis + vehAA + vehMRLS);
[[staticATmalos,staticAAmalos],6] call fn_InitNumberAssets;
[[staticATmuyMalos,staticAAmuymalos],12] call fn_InitNumberAssets;
[vehNATOAPC,0] call fn_InitNumberAssets;
[vehCSATAPC,10] call fn_InitNumberAssets;
[vehNATOTank,0] call fn_InitNumberAssets;
[vehCSATTank,5] call fn_InitNumberAssets;
[vehNATOAA,0] call fn_InitNumberAssets;
[vehCSATAA,5] call fn_InitNumberAssets;
[[vehNATOBoat],3] call fn_InitNumberAssets;
[[vehCSATBoat],3] call fn_InitNumberAssets;
[vehNATOPlane,2] call fn_InitNumberAssets;
[vehCSATPlane,10] call fn_InitNumberAssets;
[vehNATOPlaneAA,2] call fn_InitNumberAssets;
[vehCSATPlaneAA,10] call fn_InitNumberAssets;
[vehNATOTransportHelis - [vehNATOPatrolHeli],3] call fn_InitNumberAssets;
[vehCSATTransportHelis - [vehCSATPatrolHeli],30] call fn_InitNumberAssets;
[vehNATOAttackHelis,0] call fn_InitNumberAssets;
[vehCSATAttackHelis,20] call fn_InitNumberAssets;
[vehNATOMRLS,0] call fn_InitNumberAssets;
[vehCSATMRLS,5] call fn_InitNumberAssets;
		...
		if !(isMultiplayer) then
			...
			if (worldName == "Tanoa") then
				...
				if (worldName == "Altis") then
					{
					unlockedWeapons pushBack "launch_MRAWS_sand_rail_F";
					unlockedWeapons pushBack "launch_RPG7_F";
					unlockedAT = ["launch_MRAWS_sand_rail_F","launch_RPG7_F"];
