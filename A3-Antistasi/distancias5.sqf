    ...
    if (lados getVariable [_marcador,sideUnknown] == malos) then {
	    if (spawner getVariable _marcador != 0) then {
		    if (spawner getVariable _marcador == 2) then {
			    if (({if (_x distance2D _posicionMRK < distanciaSPWN) exitWith {1}} count _greenfor > 0) or
			        ({if ((_x distance2D _posicionMRK < distanciaSPWN2)) exitWith {1}} count _opfor > 0) or
			        ({if ((isPlayer _x) and (_x distance2D _posicionMRK < distanciaSPWN2)) exitWith {1}} count _blufor > 0) or
			        (_marcador in forcedSpawn)) then {
	    ...
	    if (lados getVariable [_marcador,sideUnknown] == buenos) then {
		    if (spawner getVariable _marcador != 0) then {
			    if (spawner getVariable _marcador == 2) then {
				    if (({if (_x distance2D _posicionMRK < distanciaSPWN) exitWith {1}} count _blufor > 0) or
				        ({if (_x distance2D _posicionMRK < distanciaSPWN) exitWith {1}} count _opfor > 0) or
				        ({if (((_x getVariable ["owner",objNull]) == _x) and (_x distance2D _posicionMRK < distanciaSPWN2)) exitWith {1}} count _greenfor > 0) or
				        (_marcador in forcedSpawn)) then {
					    spawner setVariable [_marcador,0,true];
					    if (_marcador in ciudades) then {
